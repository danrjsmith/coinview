 ...
 **/
export declare type ExcludeIfContainsNever<a, b> = b extends Map<any, any> | Set<any> ? a : b extends readonly [any, ...any] ? ExcludeNeverObject<a, b, '0' | '1' | '2' | '3' | '4'> : b extends any[] ? ExcludeNeverObject<a, b, number> : ExcludeNeverObject<a, b, string>;
declare type ExcludeNeverObject<a, b, keyConstraint = unknown> = a extends any ? {
    [k in keyConstraint & keyof b & keyof a]-?: [a[k]] extends [never] ? 'exclude' : 'include';
}[keyConstraint & keyof b & keyof a] extends infer includeOrExclude ? (includeOrExclude extends 'include' ? 'include' extends includeOrExclude ? true : false : false) extends true ? a : never : never : never;
